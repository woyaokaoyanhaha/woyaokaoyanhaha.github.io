...
from sklearn.metrics import roc_auc_score, confusion_matrix, average_precision_score, precision_recall_curve
    ...
    if isinstance(model, (CustomVotingClassifier, CustomStackingClassifier)):
        ...
        def evaluate_fold(i, train_idx, val_idx):
            ...
            try:
                ...
                print(f"折{i + 1}: 准确率={acc:.4f} 精确率={prec:.4f} 召回率={rec:.4f} F1={f1:.4f} AUC={auc:.4f} AUPR={aupr:.4f}")
...
def predict_interaction(models, sample_scaled, output_dir, cold_start_type=""):
    ...
    try:
        ...
        for name, model in models.items():
            try:
                pred, prob = predict_with_model(name, model)
                predictions.append(pred)
                if prob > 0:
                    probs.append(prob)
            except:
...
def train_standard_model(df, protein_id_col, compound_id_col, label_col, X, protein_cols,
    ...
    if summary_data:
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(model_dir, 'aupr_comparison.png'), dpi=300, bbox_inches='tight')
            plt.close()
        except Exception as e:
...
def train_cold_start_model(cold_start_type, df, protein_id_col, compound_id_col, label_col, X, protein_cols,
    ...
    if summary_data:
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(cold_start_dir, 'aupr_comparison.png'), dpi=300, bbox_inches='tight')
            plt.close()
        except Exception as e:
            print(f"绘制模型比较图出错，但不影响程序继续运行")
...
def compare_cold_start_models(results_dict, output_dir):
    ...
    if best_models:
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, 'cold_start_aupr_comparison.png'), dpi=300, bbox_inches='tight')
            plt.close()
            ...
            print(f"绘制冷启动策略比较图出错，但不影响程序继续运行")
    else:
...
def compare_all_models(results_dict, output_dir):
    ...
    if best_models:
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, 'all_models_aupr_comparison.png'), dpi=300, bbox_inches='tight')
            plt.close()
            ...
            plt.savefig(os.path.join(output_dir, 'all_models_radar_chart.png'), dpi=300, bbox_inches='tight')
            plt.close()
        except Exception as e:
...
def main():
    ...
    """
    try:
        print(f"开始执行模型训练与相互作用预测 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        # 文件路径
        input_file = "核受体PsePSSM和PseACC特征.csv"
        ...
        # 同时也比较冷启动模型的性能
        cold_start_results = {k: v for k, v in all_model_results.items() if k != '标准随机分割'}
        ...
        for model_type, results in all_model_results.items():
            ...
            try:
                ...
                predictions = predict_interaction(
                    results['models'], sample_scaled, output_dir, cold_start_type=model_type
                )

                # 保存预测结果到各自的目录
                model_dir = os.path.join(output_dir, model_type)
                pd.DataFrame(predictions).to_csv(
                    os.path.join(model_dir, f'Q13133_EHDPP_prediction.csv'),
                    index=False, encoding='utf-8-sig'
                )
            except Exception as e:
                print(f"使用{model_type}模型进行预测时出错: {e}")
                continue
        ...
        for model_type, results in all_model_results.items():
            model_dir = os.path.join(output_dir, model_type)
            try:
                pred_file = os.path.join(model_dir, 'Q13133_EHDPP_prediction.csv')
                ...
                print(f"读取{model_type}预测结果出错: {e}")
        ...
        if all_predictions:
            ...
            try:
                plt.figure(figsize=(10, 6))
                ax = sns.barplot(x='模型类型', y='概率', data=combined_df)
                ...
                plot_with_chinese_font(
                    fig,
                    title='不同模型的Q13133-EHDPP相互作用概率预测',
                    xlabel='模型类型',
                    ylabel='概率'
                )
                ...
                print(f"绘制综合预测结果图出错，但不影响程序继续运行")
        ...
        try:
            best_models = {
                model_type: results.get('best_model', "未知")
                for model_type, results in all_model_results.items()
            }
            ...
            with open(os.path.join(output_dir, 'final_summary.txt'), 'w', encoding='utf-8') as f:
                ...
                for model_type, results in all_model_results.items():
                    if results.get('results') and results.get('best_model'):
