#!/usr/bin/env python3
"""
蛋白质-化合物特征提取脚本 (DTI优化版本 - AAC+DPC)
作者: woyaokaoyanhaha
版本: 13.0 (AAC+DPC优化)
日期: 2025-06-16 15:28:01
修复: 使用AAC+DPC蛋白质特征和DTI优化的分子指纹特征
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='蛋白质-化合物特征提取脚本 (DTI优化版本 - AAC+DPC)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  python3 extract_protein_compound_features_v13.py data.csv
  python3 extract_protein_compound_features_v13.py data.csv -o ./output
  python3 extract_protein_compound_features_v13.py --list-resume
        """
    )
...
def main():
    ...
    try:
        ...
        if not args.input_csv:
            print("❌ 缺少必需参数")
            print("使用方法:")
            print("  python3 extract_protein_compound_features_v13.py <input.csv>")
            print("  python3 extract_protein_compound_features_v13.py --list-resume")
            return 1
        ...
        if 'work_dir' in locals():
            print(f"python3 {sys.argv[0]} {args.input_csv} -r {work_dir}")
        return 1
    except Exception as e:
        print(f"\n❌ 发生错误: {e}")
        traceback.print_exc()
