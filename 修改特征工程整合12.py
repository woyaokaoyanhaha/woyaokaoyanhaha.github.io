...
# 输入输出设置
INPUT_FILE = "Combine_BioAssay_combined_features.csv"  # 训练数据文件路径
OUTPUT_DIR = "Combine_BioAssay多模型"  # 输出目录，为空则自动生成以日期时间命名的目录
...
def improved_feature_selection(X_train, y_train, X_test, feature_names, max_features=100):
    """改进的特征选择，更加严格和有效"""
    ...
    try:
        ...
        if len(feature_vars) > 10:
            variance_threshold = feature_vars.quantile(0.2)  # 保留方差前80%的特征
        else:
        ...
        var_features = [feature_names[i] for i in range(len(feature_names)) if var_mask[i]]
        print(f"方差筛选保留特征: {len(var_features)}/{len(feature_names)} (阈值: {variance_threshold:.6f})")
    ...
    try:
        if len(selected_features) > max_features:
            ...
            # 选择显著性最高的特征
            k_best = min(max_features, len(selected_features))
            selector_f = SelectKBest(f_classif, k=k_best)
            X_f = selector_f.fit_transform(X_train_var, y_train)
            f_mask = selector_f.get_support()
            f_features = [selected_features[i] for i in range(len(selected_features)) if f_mask[i]]
            print(f"F检验保留特征: {len(f_features)}")
    ...
    try:
        if len(selected_features) > max_features:
            ...
            top_indices = importance_indices[:min(max_features, len(selected_features))]

            important_features = [selected_features[i] for i in top_indices]

            print(f"随机森林重要性筛选保留特征: {len(important_features)}")
    ...
    try:
        if len(selected_features) > 10:
            ...
            print(f"相关性筛选移除{len(to_drop)}个高度相关特征，保留: {len(selected_features)}")
    except Exception as e:
        print(f"相关性筛选失败: {e}")
    ...
    if len(selected_features) == 0:
        ...
        print(f"特征数量过多，截取前{max_features}个")
...
def improved_feature_scaling(X_train, X_test):
    ...
    imputer = SimpleImputer(strategy='median')  # 使用中位数更稳健
    ...
    for name, scaler in scalers.items():
        try:
            ...
            # 计算质量分数（越接近标准正态分布越好）
            std_score = np.mean(np.abs(train_std - 1))  # 标准差应接近1
            mean_score = np.mean(np.abs(train_mean))  # 均值应接近0
    ...
    # 最终检查
    best_train_scaled = np.nan_to_num(best_train_scaled, nan=0.0, posinf=0.0, neginf=0.0)
    best_test_scaled = np.nan_to_num(best_test_scaled, nan=0.0, posinf=0.0, neginf=0.0)

    return best_scaler, best_train_scaled, best_test_scaled, imputer


def diagnose_feature_quality(X_train_scaled, X_test_scaled, feature_names):
    """诊断特征质量"""
    print("\n=== 特征质量诊断 ===")

    # 检查特征的方差
    train_vars = np.var(X_train_scaled, axis=0)
    zero_var_count = np.sum(train_vars < 1e-10)
    print(f"零方差特征数量: {zero_var_count}/{len(feature_names)}")

    # 检查特征的分布
    train_means = np.mean(X_train_scaled, axis=0)
    train_stds = np.std(X_train_scaled, axis=0)

    print(f"特征均值范围: [{np.min(train_means):.4f}, {np.max(train_means):.4f}]")
    print(f"特征标准差范围: [{np.min(train_stds):.4f}, {np.max(train_stds):.4f}]")

    # 检查异常值
    extreme_features = np.sum(np.abs(X_train_scaled) > 5, axis=0)
    extreme_count = np.sum(extreme_features > len(X_train_scaled) * 0.01)
    print(f"包含极值的特征数量: {extreme_count}")

    # 检查特征间差异
    feature_diffs = []
    for i in range(min(10, len(X_train_scaled))):
        for j in range(i + 1, min(10, len(X_train_scaled))):
            diff = np.sum(np.abs(X_train_scaled[i] - X_train_scaled[j]))
            feature_diffs.append(diff)

    if feature_diffs:
        avg_diff = np.mean(feature_diffs)
        print(f"样本间平均特征差异: {avg_diff:.4f}")

    return {
        'zero_var_features': zero_var_count,
        'mean_range': (np.min(train_means), np.max(train_means)),
        'std_range': (np.min(train_stds), np.max(train_stds)),
        'extreme_features': extreme_count,
        'sample_differences': np.mean(feature_diffs) if feature_diffs else 0
    }
...
def optimize_features(X_train, X_test, y_train, protein_cols, compound_cols):
    ...
    X_train_selected, X_test_selected, selected_features = improved_feature_selection(
    ...
    # 4. 特征质量诊断
    quality_info = diagnose_feature_quality(X_train_scaled, X_test_scaled, selected_features)
    ...
    print(f"特征工程完成: {original_feature_count} -> {len(selected_features)}")
    print("=== 特征工程优化完成 ===\n")

    return X_train_scaled, X_test_scaled, feature_pipeline


def improved_process_specific_sample(sample_df, feature_pipeline):
    """改进的特定样本处理"""
    try:
        print("处理预测样本...")

        # 1. 使用相同的特征增强流程
        sample_enhanced = improved_enhance_features(
            sample_df,
            feature_pipeline['protein_cols'],
        ...
        if missing_features:
            print(f"警告: {len(missing_features)}个特征在样本中缺失，已填充为0")
        ...
        if feature_pipeline['scaler'] is not None:
            sample_scaled = feature_pipeline['scaler'].transform(sample_imputed)
        else:
            sample_scaled = sample_imputed
        ...
        # 6. 诊断处理后的样本
        print(f"处理后样本特征统计:")
        print(f"  均值: {np.mean(sample_scaled):.6f}")
        print(f"  标准差: {np.std(sample_scaled):.6f}")
        print(f"  最小值: {np.min(sample_scaled):.6f}")
        print(f"  最大值: {np.max(sample_scaled):.6f}")
...
def process_specific_sample(sample_df, feature_pipeline):
    """使用特征工程管道处理样本（使用改进版本）"""
    return improved_process_specific_sample(sample_df, feature_pipeline)
...
def process_specific_sample(sample_df, feature_pipeline):
    ...
    try:
        # 1. 特征增强
        sample_enhanced = improved_enhance_features(sample_df, feature_pipeline['protein_cols'],
                                                    feature_pipeline['compound_cols'])
