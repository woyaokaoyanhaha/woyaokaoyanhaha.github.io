...
def compare_all_models(results_dict, output_dir):
    ...
    if best_models:
        ...
        print("\n所有模型策略最佳模型性能比较:")
        print("=" * 100)
        print(f"{'模型类型':<15}{'最佳模型':<20}{'准确率':<10}{'精确率':<10}{'召回率':<10}{'F1分数':<10}{'AUC':<10}")
        print("-" * 100)
        for idx, row in comparison_df.iterrows():
            print(
                f"{row['模型类型']:<15}{row['最佳模型']:<20}{row['准确率']:.4f}{row['精确率']:<10.4f}{row['召回率']:<10.4f}{row['F1分数']:<10.4f}{row['AUC']:<10.4f}")
        ...
        try:
            ...
            print(f"绘制模型策略比较图出错，但不影响程序继续运行")
    else:
...
def main():
    ...
    try:
        ...
        print("\n\n所有模型在各种策略下的测试集性能总结:")
        print("=" * 100)
        print(f"{'模型类型':<15}{'模型':<20}{'准确率':<10}{'精确率':<10}{'召回率':<10}{'F1分数':<10}{'AUC':<10}")
        print("-" * 100)
